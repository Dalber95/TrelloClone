            ...
            if((context as TaskListActivity).assignedMemberDetailList.size > 0){
                ...
                for(i in context.assignedMemberDetailList.indices){
                    for(j in model.assignedTo){
                        if(context.assignedMemberDetailList[i].id == j){
                            val selectedMembers = SelectedMembers(context.assignedMemberDetailList[i].id, context.assignedMemberDetailList[i].image)
                ...
                if (selectedMembersList.size > 0){
                    if (selectedMembersList.size == 1 && selectedMembersList[0].id == model.createdBy){
                        ...
                        holder.itemView.rvCardSelectedMembersList.layoutManager = GridLayoutManager(context, 4)
